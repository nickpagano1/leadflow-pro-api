...
def test_login():
    ...
    try:
        ...
        # Get the user
        cursor = conn.execute("SELECT email, password_hash, is_active FROM agents WHERE email = ?", ("npagano@besenpartners.com",))
        ...
        if agent:
            ...
            try:
                ...
                print("Active: Unknown (column might not exist)")
            
            # Test the password
            test_password = "password123"
            if verify_password(test_password, agent['password_hash']):
                print(f"✅ Password verification SUCCESS for 'password123'")
            else:
                print(f"❌ Password verification FAILED for 'password123'")
